...
class Solver(object):
    ...
    def train(self):
        ...
        for e in range(continue_epochs, self.train_config.n_epoch):
            ...
            for batch in self.train_data_loader:
                ...
                # if e == 10 and order_center_loss_.__len__() == 22:
    ...
    # 定义定位损失函数, 非加权
    # def ordinal_loss(self, p_class, target, num_classes):
    #     probas = torch.softmax(p_class, dim=1)
    #     target = target.view(-1, 1)
    #     class_range = torch.arange(0, num_classes).view(1, -1).to(p_class.device)
    #     loss = (class_range - target) ** 2
    #     loss = (loss * probas).sum(dim=1).mean()
    #
    #     return loss
    ...
    def ordinal_loss(self, p_class, target, num_classes):
        ...
        target = target.view(-1, 1)
        class_range = torch.arange(0, num_classes).view(1, -1).to(p_class.device)
        class_weight = (class_range - target) ** 2
        # 计算每个类别的样本个数
        class_counts = torch.zeros(num_classes).to(p_class.device)
        for i in range(num_classes):
            class_counts[i] = (target == i).sum()
        # 计算权重（样本数的倒数）
        weights = 1.0 / (class_counts + 1e-6)  # 添加一个小值以防止除以零
        weights = weights / weights.sum()  # 归一化权重
        # 根据每个样本的类别，赋予相应的权重
        sample_weights = weights[target.view(-1).long()]

        loss = (class_weight * probas).sum(dim=1) * sample_weights
        # return loss.mean()
        return loss.mean()
    ...
    def interval_center_loss(self, pred, target, num_classes):
        interval_center = target * 5 + 2
        target = target.view(-1, 1)
        # 计算每个类别的样本个数
        class_counts = torch.zeros(num_classes).to(pred.device)
        for i in range(num_classes):
            class_counts[i] = (target == i).sum()
        # 计算权重（样本数的倒数） 类别样本数量的倒数归一化，使所有权重之和为 1
        weights = 1.0 / (class_counts + 1e-6)  # 添加一个小值以防止除以零
        weights = weights / weights.sum()  # 归一化权重
        # 根据类别标签为每个样本分配权重
        sample_weights = weights[target.view(-1).long()]
        # loss = (pred - interval_center) ** 2
        loss = torch.abs(pred - interval_center)
        # 将权重应用于每个样本的损失
        weighted_loss = loss.squeeze() * sample_weights
        return weighted_loss.sum()


    def gaussian_label_distribution(self, labels, num_classes=5, uncertainty=None, sigmas=1):
        batch_size = labels.size(0)
        labels = labels.expand(batch_size, num_classes)
        classes = torch.arange(0, num_classes).unsqueeze(0).repeat(batch_size, 1).cuda()
        sigmas = torch.ones_like(labels)*sigmas  # 初始方差
        if uncertainty is None:
            uncertainty = torch.ones_like(labels)  # If no sigmas provided, default to 1.0
        else:
            uncertainty = uncertainty.repeat(1, num_classes)
        gaussian_labels = torch.exp(-((labels - classes) ** 2) / (2 * (uncertainty * sigmas) ** 2))
        gaussian_labels = gaussian_labels / gaussian_labels.sum(dim=1, keepdim=True)
        return gaussian_labels


    def gaussian_kl_div_loss(self, y_pred, y_true):
        epsilon = 1e-9  # A small constant to avoid log(0)
        y_true = torch.clamp(y_true, min=epsilon)

        log_y_pred = F.log_softmax(y_pred, dim=1)
        return F.kl_div(log_y_pred, y_true, reduction='batchmean')


    def scale_uncertainty(self, uncertainty, min_val=0.5, max_val=2.0):
        min_uncertainty = torch.min(uncertainty)
        max_uncertainty = torch.max(uncertainty)
        scaled_sigma = ((uncertainty - min_uncertainty) / (max_uncertainty - min_uncertainty)) * (
