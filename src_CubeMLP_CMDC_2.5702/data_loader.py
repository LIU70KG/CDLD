...
from create_dataset import DAIC_WOZ, CMDC
...
class MSADataset(Dataset):
    def __init__(self, config):
        ...
        ## Fetch datase
        if "cmdc" in str(config.data_dir).lower():
            dataset = CMDC(config)
        elif "daic-woz" in str(config.data_dir).lower():
        ...
        if "daic-woz" in str(config.data_dir).lower():
            ...
            config.txt_size = self.data[0][0][0].shape[1]
