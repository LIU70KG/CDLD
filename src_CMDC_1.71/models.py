...
class MISA_CMDC(nn.Module):
    def __init__(self, config):
        ...
        self.private_t.add_module('private_t_activation_1_norm', nn.LayerNorm(config.hidden_size))
        ...
        ####################################################################################
        # ----------将各样本特征映射到类特征中----------------
        self.Dimensionality_reduction = nn.Linear(in_features=self.config.hidden_size * 3, out_features=128)
        self.features_to_center = nn.Linear(in_features=128, out_features=32)
        # -----------将类特征去预测类别--------------------
        ...
        # self.features_to_center.add_module('fusion_layer_1', nn.Linear(in_features=128, out_features=128))
        # self.features_to_center.add_module('fusion_layer_1_activation', nn.Sigmoid())
        ...
        self.centers = nn.Parameter(torch.zeros(5, 32))  # 设置全是0？
        self.fc_shift = nn.Linear(128+32, output_size)
        # ----------直接回归分数-------------------
    ...
    def alignment(self, sentences, visual, acoustic, lengths, bert_sent, bert_sent_type, bert_sent_mask, label_area, mode):
        ...
        h = torch.stack((self.utt_t_orig, self.utt_v_orig, self.utt_a_orig,), dim=0)
        ...
        if mode == "train":
            ...
            # features_shift = h - centers_batch
            p_shift = self.fc_shift(features_shift)
            # p_shift = self.fc_shift(sim)
            p_shift = torch.clamp(p_shift, min=-2.0, max=2.0)
    ...
    def forward(self, sentences, video, acoustic, lengths, bert_sent, bert_sent_type, bert_sent_mask, label_area, mode):
        batch_size = lengths.size(0)
        if mode == "train":
            pred, pred_center_score, p_class, p_shifting, uncertainty, order_center_loss= self.alignment(sentences, video, acoustic, lengths, bert_sent, bert_sent_type, bert_sent_mask, label_area, mode)
            return pred, pred_center_score, p_class, p_shifting, uncertainty, order_center_loss

        if mode == "test":
            pred, pred_center_score, p_class, p_shifting = self.alignment(sentences, video, acoustic, lengths,
                                                                                       bert_sent, bert_sent_type,
                                                                                       bert_sent_mask, label_area, mode)
            return pred, pred_center_score, p_class, p_shifting

        if mode == "tsne":
            h = self.alignment(sentences, video, acoustic, lengths,
                                                                                       bert_sent, bert_sent_type,
                                                                                       bert_sent_mask, label_area, mode)
            return h
...
class Simple_Fusion_Network(nn.Module):
    def __init__(self, config):
        ...
        # ----------将各样本特征映射到类特征中----------------
        self.Dimensionality_reduction = nn.Linear(in_features=self.config.hidden_size * 3, out_features=32)
        ...
        # self.fc_shift = nn.Linear(32, output_size)
    ...
    def alignment(self, sentences, visual, acoustic, lengths, bert_sent, bert_sent_type, bert_sent_mask, label_area, mode):
        ...
        # Projecting to same sized space
        self.utt_t_orig = utterance_t = self.project_t(utterance_text)
        self.utt_v_orig = utterance_v = self.project_v(utterance_video)
        self.utt_a_orig = utterance_a = self.project_a(utterance_audio)
    ...
    def forward(self, sentences, video, acoustic, lengths, bert_sent, bert_sent_type, bert_sent_mask, label_area, mode):
        ...
        if mode == "tsne":
            features_class = self.alignment(sentences, video, acoustic, lengths,
                                                                                       bert_sent, bert_sent_type,
                                                                                       bert_sent_mask, label_area, mode)
            return features_class