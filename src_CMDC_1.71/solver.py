...
class Solver(object):
    ...
    def train(self):
        ...
        for e in range(0, self.train_config.n_epoch):
            ...
            if flag == 1:
                ...
                # self.picture(src_d=f'checkpoints/model_{self.train_config.name}.std', it=e)
    ...
    def picture(self, src_d=None, it=None):
        Feature, Label_area = [], []

        dataloader = self.test_data_loader

        self.model.load_state_dict(torch.load(src_d))
        ...
        colors = ['green', 'blue', 'yellow', 'orange', 'red']
        features = np.concatenate(Feature, axis=0)
        arr_min = np.min(features)
        arr_max = np.max(features)
        features = (features - arr_min) / (arr_max - arr_min)
        labels = np.concatenate(Label_area, axis=0)
        # 将标签和特征组合在一起，便于排序
        ...
        plt.tight_layout()
        # plt.show()
        plt.savefig(f'./figure/cmdc_all_{it}.png', bbox_inches='tight')
        plt.close("all")
